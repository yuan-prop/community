...
import majiang.comunity.enums.NotificationStatusEnum;
import majiang.comunity.exception.CustomizeErrorCode;
import majiang.comunity.exception.CustomizeException;
import majiang.comunity.mapper.*;
import majiang.comunity.model.*;
...
public class CommentService {
    ...
    public void insert(Comment comment, User commentator) {
        ...
        if(comment.getType() == CommentTypeEnum.CONTENT.getType()){
            ...
            comment.setCommentCount(0);//评论初始值
            commentMapper.insert(comment);
            question.setCommentCount(1);
            questionExtraMapper.incCommentCount(question);

            //创建通知
            createNotify(comment, question.getCreator(), commentator.getName(), question.getTitle(), NotificationTypeEnum.REPLY_QUESITON, question.getId());
        }
    }
